...
u8	APhaseReceBuf[PACK_SIZE] = {0};							//A相数据采集缓存
u8	BPhaseReceBuf[PACK_SIZE] = {0};							//B相数据采集缓存

u8 	EnableCorrectRegBuf[PACK_SIZE]   = {0x00,0x00,0x5a};	//选择校表寄存器
u8 	EnableMeterRegBuf[PACK_SIZE]     = {0x00,0x00,0x5b};	//选择计量寄存器

//校表寄存器的设置值(该类寄存器的字长为2Byte，故要设置的值写在低两字节中)
u8 	ModeConfigBuf[PACK_SIZE] 		 = {0x00,0x89,0xfe};	//设置电压/电流ADC通道
u8 	AnalogModuleConfigBuf[PACK_SIZE] = {0x00,0x35,0x21};	//设置高通滤波(关闭电压&&电流通道数字高通滤波器)
...
/************************************
	...
	*返回值：	无
*************************************/
static void HT7038ResetSet(u8 state)
{
    if( state == ENABLE )
    {
		RST_L();			//将复位引脚置低
