...
/************************************************************************************
	...
	*@param		protocol: The socket to chose.使用的协议类型
...
/***********************************************************************
	*@brief		This function close the socket and 
				parameter is "s" which represent the socket number
	*@param		s: socket number.
	*@return  	None
************************************************************************/
void close(SOCKET s)
{
    IINCHIP_WRITE( Sn_CR(s), Sn_CR_CLOSE);	//关闭Socket s
	
    while( IINCHIP_READ( Sn_CR(s) ) );		//wait to process the command...
	
    IINCHIP_WRITE( Sn_IR(s), 0xFF);	  		//将Sn_IR位清零(主机将相应的位置'1')
}
...
/************************************************************************************
	*@brief		This function is an application I/F function which is used to 
				...
				Unlike TCP transmission, The peer's destination address and 
				the port is needed.
	*@param		s: socket number.
	*@param		buf: data buffer to send.
	*@param		len: data length.
...
/***********************************************************************************
	*@brief   	This function is an application I/F function which is used
				to receive the data in other then
				TCP mode. This function is used to receive UDP, IP_RAW and
				MAC_RAW mode, and handle the header as well.
	*@param		s: socket number.
	*@param		buf: data buffer to receive.
	*@param		len: data length.
...
uint16 recvfrom(SOCKET s, uint8 * buf, uint16 len, uint8 * addr, uint16 *port)
{
    ...
    if ( len>0 )
    {
        ...
        switch (IINCHIP_READ(Sn_MR(s) ) & 0x07)		//0000 0111
        {
			case Sn_MR_UDP :
				wiz_read_buf(addrbsb, head, 0x08);	//从W5500中读取8个Byte的数据
				ptr += 8;
				//IP地址,
				addr[0]  = head[0];
				addr[1]  = head[1];
        ...
        //RECV命令通过使用接收读指针寄存器来完成在Socket n接收缓存中接收数据的过程
        IINCHIP_WRITE( Sn_CR(s), Sn_CR_RECV);
