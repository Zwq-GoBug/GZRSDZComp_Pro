...
void send_DHCP_DISCOVER(void)
{
    ...
    uint16 	i = 0;
    uint8 	host_name[12]; 						//the host name modified
    //*((uint32*)DHCP_SIP)= 0;
...
void send_DHCP_REQUEST(void)
{
    ...
    pRIPMSG->OPT[i++] = hostName;
    //set the host name
    sprintf( (char*)host_name, (char*)"%.4s-%02X%02X%02X", 
			 DEVICE_ID, SRC_MAC_ADDR[3], SRC_MAC_ADDR[4], SRC_MAC_ADDR[5]);
    pRIPMSG->OPT[i++] = (uint8)strlen((char*)host_name);
    strcpy((char*) & (pRIPMSG->OPT[i]), (char*)host_name);
...
uint8 parseDHCPMSG(uint16 length)
{
    ...
    while ( p < e )
    {
        switch ( *p++ )
        {
			...
			#ifdef DHCP_DEBUG
				printf("subnetMask : ");
				printf("%u.%u.%u.%u\r\n", GET_SN_MASK[0], GET_SN_MASK[1], 
										  GET_SN_MASK[2], GET_SN_MASK[3]);
			#endif
			break;
			...
			case routersOnSubnet :
				...
				printf("%u.%u.%u.%u\r\n", GET_GW_IP[0], GET_GW_IP[1], 
										  GET_GW_IP[2], GET_GW_IP[3]);
			#endif
			break;
			...
			case dhcpServerIdentifier :
				opt_len = *p++;
			#ifdef DHCP_DEBUG
				printf("DHCP_SIP : %u.%u.%u.%u\r\n", DHCP_SIP[0], DHCP_SIP[1], 
												     DHCP_SIP[2], DHCP_SIP[3]);
			#endif
						...
						*((uint32*)DHCP_SIP) == *((uint32*)svr_addr) )
				{
					...
					printf("%u.%u.%u.%u\r\n", DHCP_REAL_SIP[0], DHCP_REAL_SIP[1], 
											  DHCP_REAL_SIP[2], DHCP_REAL_SIP[3]);
				#endif
				}
				else
				{
...
/**************************************************************************
	*@brief	 	检查DHCP状态
	*@param		s: socket数
	*@return	返回得到的DHCP状态	DHCP_RET_NONE: 未获取到IP地址
									DHCP_RET_TIMEOUT:超时
...
uint8 check_DHCP_state(SOCKET s)
{
    ...
    {
        if ( (len = getSn_RX_RSR(s)) > 0 )	//读取空闲接收缓存寄存器的值(得到接收数据包的大小)
        {
            type = parseDHCPMSG(len);		//解析收到的封包类型
        }
    }
    //socket处于关闭状态，重新初始化socket
    else
    {
...
void init_dhcp_client(void)
{
    ...
    iinchip_init();		//初始化W5500内部寄存器
    setSUBR(ip_0);		//重置子网掩码
    setGAR(ip_0);		//重置网关地址
    setSIPR(ip_0);		//重置源IP地址

    //串口打印源MAC地址
    printf("mac=%02x:%02x:%02x:%02x:%02x:%02x\r\n", SRC_MAC_ADDR[0], SRC_MAC_ADDR[1],
...
void do_dhcp(void)
{
    ...
    dhcp_timer_init();			//初始化DHCP定时器2(定时时间为：1ms)
    if(Conflict_flag == 1)
    {
